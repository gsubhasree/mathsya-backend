...
fetchChannelConfig() {
  ...
  set -x
  peer channel fetch config config_block.pb -o orderer.mathsya.tech:7050 --ordererTLSHostnameOverride orderer.mathsya.tech -c ${CHANNEL_NAME} --tls --cafile $ORDERER_CA
  { set +x; } 2>/dev/null
