...
function createOrg() {
    ...
    infoln "Copying admin private key for ${name}"
    docker run --rm -v "$(pwd):/data"  -u $(id -u):$(id -g) busybox sh -c "cd /data && cp organizations/peerOrganizations/${subdomain}.${domain}/users/Admin@${subdomain}.${domain}/msp/keystore/* organizations/peerOrganizations/${subdomain}.${domain}/users/Admin@${subdomain}.${domain}/msp/keystore/priv_sk"
